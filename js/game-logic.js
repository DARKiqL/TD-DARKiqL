...
function updateDPSDisplays() {
    ...
    hpDisplay.textContent = `DPS کل فعلی: ${totalDps.toFixed(2)}`;

    if (selectedPlacedTower) {
        const selectedDps = damageLog
            .filter(log => log.towerId === selectedPlacedTower.id)
            .reduce((sum, log) => sum + log.amount, 0);
        waveDisplay.textContent = `DPS این برج: ${selectedDps.toFixed(2)}`;
    } else {
        waveDisplay.textContent = 'DPS این برج: در انتظار انتخاب';
    }
}
...
function syncPauseButton() {
    ...
    pauseButton.title = isPaused ? 'ادامه بازی' : 'توقف بازی';
    pauseButton.querySelector('span').textContent = isPaused ? 'ادامه' : 'توقف';
...
document.querySelectorAll('.keybinding-capture').forEach(button => {
    button.addEventListener('click', () => {
        ...
        button.setAttribute('aria-label', 'کلید جدید را فشار دهید، برای لغو Escape را بزنید');
        button.focus();
    });
...
function updateWaveCountdownBubble() {
    ...
    waveCountdownBubble.textContent = wave === 0
        ? `${seconds} ثانیه تا شروع`
        : `${seconds} ثانیه تا موج بعدی`;
}
...
function buildSpeedControls() {
    ...
    speedControls.innerHTML = speeds.map((sp, i) =>
        `<button type="button" class="speed-opt" data-speed-index="${i}" aria-label="سرعت ${sp} برابر">${sp}×</button>`
    ).join('');
}
...
function getTowerSkillAction(tower) {
    if (tower.type === 'electricCore' && tower.level >= 1) return { action: 'electric-skill', label: 'اورکلاک', color: '#4dd0e1' };
    if (tower.type === 'musicStand' && tower.level >= 2) return { action: 'music-skill', label: 'الکترو', color: '#e91e63' };
    if (tower.type === 'pursuit' && tower.level === 4) return { action: 'pursuit-overload', label: 'اضافه بار', color: '#ff5252' };
    return null;
}
...
function renderSkillRail() {
    ...
    skillRail.innerHTML = entries.map(({ tower, skill }, index) => {
        ...
        const ready = !cooling;
        const countdown = active ? (tower.overloadActive ? `${tower.overloadMissilesRemaining} عدد` : Math.ceil(tower.skillActiveTimer / 60)) : Math.ceil(tower.skillCooldown / 60);
...
function updateUI() {
    ...
    moneyDisplay.textContent = `پول: ${money}`;
    ...
    if (!isTestMode) {
        ...
        hpDisplay.textContent = `جان پایگاه: ${hp}`;
        waveDisplay.textContent = `موج: ${wave}/${FINAL_WAVE}`;
...
function renderReplayCommandPanel() {
    ...
    towerSelectionDiv.innerHTML = `<section id="replay-command-panel" role="status" aria-live="polite"><div class="replay-command-loader" aria-hidden="true"><span></span><span></span><span></span></div><div class="replay-command-copy"><h4>اجرای فرماندهی نماینده</h4><p>نماینده ممکن است اشتباه کند</p></div>${replayOutdated ? '<p class="replay-command-stale">نسخه بازپخش قدیمی است</p>' : ''}<button id="replay-return-btn" class="action-btn" type="button">← بازگشت به منوی اصلی</button></section>`;
...
function renderTowerTargetSelector(tower, readOnly = false) {
    ...
    if (readOnly) {
        return `<div class="target-selector target-selector-readonly" aria-label="حالت هدف فعلی"><span class="target-selector-label">هدف: ${currentTargetMode}</span></div>`;
    }
    return `<div class="target-selector" aria-label="انتخاب هدف"><button type="button" class="target-cycle" data-tower-action="target-prev" aria-label="حالت هدف قبلی">‹</button><span class="target-selector-label">هدف: ${currentTargetMode}</span><button type="button" class="target-cycle" data-tower-action="target-next" aria-label="حالت هدف بعدی" data-target-index="${targetIndex}">›</button></div>`;
}

function formatTowerTotalDamage(totalDamage) {
    return Math.max(0, Number(totalDamage) || 0).toLocaleString('fa-IR', { maximumFractionDigits: 1 });
}

function buildTowerInfoHeader(tower, title) {
    const damageHTML = `<span class="tower-total-damage">آسیب تجمعی: <span>${formatTowerTotalDamage(tower.totalDamage)}</span></span>`;
...
function updateTowerInfoPanel() {
    if (selectedPlacedTower) {
        ...
        if (tower.type === 'destroyer') {
            ...
            infoHTML += `<li>وضعیت: ${translateDestroyerState(tower.destroyerState)}</li>`;
            infoHTML += `<li>لیزر: پول×1.5</li>`;
            infoHTML += `<li>گسترش: پول×0.05</li>`;
            infoHTML += `<li>برد: ${tower.range}</li>`;
            infoHTML += `<li>سرعت حمله: 1/ثانیه</li>`;
            infoHTML += `</ul>`;
            if (isReplayMode) {
            ...
            if (supportsTargetSelection(tower)) {
                const targetIndex = TARGET_MODES.indexOf(tower.targetMode);
                const currentTargetMode = TARGET_MODE_LABELS[tower.targetMode] || TARGET_MODE_LABELS.closest;
                infoHTML += `<div class="target-selector" aria-label="انتخاب هدف"><button type="button" class="target-cycle" data-tower-action="target-prev" aria-label="حالت هدف قبلی">‹</button><span class="target-selector-label">هدف: ${currentTargetMode}</span><button type="button" class="target-cycle" data-tower-action="target-next" aria-label="حالت هدف بعدی" data-target-index="${targetIndex}">›</button></div>`;
            }

            let deployBtnHTML;
            if (!isTestMode && wave < tower.cooldownWave) {
                deployBtnHTML = `<button class="action-btn" id="deploy-btn" data-tower-action="deploy" disabled>در حال خنک‌سازی (تا موج ${tower.cooldownWave})</button>`;
            } else if (tower.destroyerState === 'closed') {
                deployBtnHTML = `<button class="action-btn" id="deploy-btn" data-tower-action="deploy" ${money <= 0 ? 'disabled' : ''}>استقرار</button>`;
            } else if (tower.destroyerState === 'active' || tower.destroyerState === 'opening' || tower.destroyerState === 'closing') {
                deployBtnHTML = `<button class="action-btn" id="deploy-btn" data-tower-action="deploy" disabled>در حال حمله... (${Math.ceil(tower.activeTimer / 60)}s)</button>`;
            }
            ...
            const sellDisabled = isWaveCooldown || isActiveState;
                ...
                infoHTML += `<button class="action-btn" id="sell-btn" data-tower-action="sell" ${sellDisabled ? 'disabled' : ''}>فروش (<span class="cost">${tower.sell()}</span>)</button></div>`;
            }
            ...
            if (tower.type === 'gatlingGun' && level === 4) {
                levelDisplay = `EX (مرتبه: ${toRoman(tower.rank) || '0'})`;
            } else {
                levelDisplay = (isExTower && level === 4) ? 'EX' : level + 1;
            }

            let infoHTML = `${buildTowerInfoHeader(tower, `${data.name} (سطح ${levelDisplay})`)}<ul>`;
            ...
            Object.keys(currentStats).forEach(stat => {
                ...
                if (stat === 'damage' && tower.damage > 0) {
                    ...
                    if (['blast', 'slow', 'missileSilo', 'gravityBeacon', 'gamma', 'spotlight', 'boomerang', 'frostPunish'].includes(tower.type)) {
                        ...
                        if (totalMultiplier > 1.01) {
                            statValue = `${baseRoundsPerSecond.toFixed(1)} دور/ثانیه <span class="stat-buffed">(${finalRoundsPerSecond.toFixed(1)})</span>`;
                        } else {
                            statValue = `${baseRoundsPerSecond.toFixed(1)} دور/ثانیه`;
                        }
                    ...
                    infoHTML += `<li>ویژه: ${currentStats.attacksForGold} بار/+$${currentStats.goldPerProc}</li>`;
                    return;
            ...
            if (tower.type === 'missileSilo') {
                infoHTML += `<li>پرواز: ${(currentStats.flightTime / 60).toFixed(2)} ثانیه</li>`;
            }
            if (tower.type === 'gamma') {
                 infoHTML += `<li>زنجیره: 90% آسیب</li>`;
                 infoHTML += `<li><span style="color:gold;">زنجیره:</span> همگام‌سازی کاهش‌اثرها، هر نوع +10% آسیب</li>`;
            }

            if (tower.type === 'gatlingGun' && tower.level === 4) {
                infoHTML += `<li style="color: gold;">کل شلیک: ${tower.totalShotsFired}</li>`;
                infoHTML += `<li style="color: gold;">مرتبه بعدی: ${tower.rank < tower.maxRank ? 1500 - (tower.totalShotsFired % 1500) : 'MAX'}</li>`;
            }

            if (tower.type === 'arrow' && tower.level === 4) {
                infoHTML += `<li>تیر انفجاری: هر ${currentStats.specialAttackRate} بار / آسیب 30 منطقه‌ای</li>`;
                infoHTML += `<li><span style="color:gold;">تیر سیگنال:</span> هر ${currentStats.specialSignalRate} بار → باران تیر 3 ثانیه</li>`;
            }
            if (tower.type === 'cannon') {
                infoHTML += `<li>انفجار: 60% آسیب در محدوده 1 خانه</li>`;
            }
            if (tower.type === 'cannon' && tower.level === 4) {
                infoHTML += `<li><span style="color:gold;">لیزر:</span> هر ${currentStats.specialAttackRate} بار، +${currentStats.specialPercentDamage * 100}% جان فعلی / 5 برابر برای رئیس</li>`;
            }
            if (tower.type === 'blast' && tower.level === 4) {
                infoHTML += `<li>چند هدف: هر دشمن +(${currentStats.specialMultiplier}× تعداد برخورد) آسیب</li>`;
            }
            if (tower.type === 'magic') {
                infoHTML += `<li>شکستن دفاع: آسیب +15%، 3 ثانیه</li>`;
                if (tower.level === 4) {
                    infoHTML += `<li><span style="color:gold;font-weight:bold;">EX:</span> انباشت آسیب‌پذیری (50 لایه)، هر لایه +1% آسیب دریافتی</li>`;
                }
            }
            if (tower.type === 'slow' && tower.level === 4) {
                infoHTML += `<li><span style="color:gold;font-weight:bold;">EX:</span> هر 5 حمله نشان یخ اضافه می‌کند</li>`;
                infoHTML += `<li><span style="color:gold;font-weight:bold;"></span> یخ‌زدگی: معمولی 15 لایه 4 ثانیه / رئیس 40 لایه 2 ثانیه</li>`;
            }
            if (tower.type === 'tesla' && tower.level === 4) {
                infoHTML += `<li style="color:gold;font-weight:bold;">افزایش آسیب: +${(tower.damageBuff * 100).toFixed(0)}%</li>`;
            }
            if (tower.type === 'sun' && tower.level === 4) {
                infoHTML += `<li><span style="color:gold;font-weight:bold;">EX:</span> میدان +35%، خانه‌های مجاور +10-40% آسیب دریافتی</li>`;
                infoHTML += `<li>کندی در میدان -80%، مصونیت از نشان یخ</li>`;
            }
            if (tower.type === 'spotlight') {
                if (currentStats.burnPercent) {
                    infoHTML += `<li>سوختن: 0.25 ثانیه/${(currentStats.burnPercent * 100).toFixed(1)}% جان بیشینه، 6 ثانیه</li>`;
                    infoHTML += `<li>مقاومت سوختن: هر ثانیه +5%، حداکثر 50%</li>`;
                }
                if (tower.level === 4) {
                    infoHTML += `<li><span style="color:gold;font-weight:bold;">EX:</span> دوستان 25% ضربه بحرانی (تک‌هدف 2x / منطقه‌ای 1.25x)</li>`;
                }
            }
            if (tower.type === 'pursuit') {
                infoHTML += `<li>موشک: ${currentStats.missileCount} عدد${currentStats.sharedVision ? ' / دید مشترک' : ''}</li>`;
                infoHTML += `<li>بارگذاری: ${(currentStats.reloadTime / 60).toFixed(1)} ثانیه</li>`;
                if (tower.overloadActive) {
                    infoHTML += `<li style="color:#ff1744;font-weight:bold;">اضافه بار فعال: ${tower.overloadMissilesRemaining} عدد باقی‌مانده</li>`;
                } else if (tower.salvoRemaining === 0 && tower.reloadTimer > 0) {
                    infoHTML += `<li style="color:#4dd0e1;">در حال بارگذاری: ${Math.ceil(tower.reloadTimer / 60)} ثانیه</li>`;
                } else if (tower.salvoRemaining > 0) {
                    infoHTML += `<li style="color:#ff5252;">در حال شلیک: ${tower.salvoRemaining} عدد باقی‌مانده</li>`;
                } else {
                    infoHTML += `<li style="color:#4caf50;">آماده</li>`;
                }
                if (tower.level === 4) {
                    if (tower.overloadActive) {
                        infoHTML += `<li style="color:#ff1744;font-weight:bold;">اضافه بار EX: ${tower.overloadMissilesRemaining} عدد / +20%</li>`;
                    } else if (tower.skillCooldown > 0) {
                        infoHTML += `<li style="color:#ff5252;">خنک‌سازی اضافه بار EX: ${Math.ceil(tower.skillCooldown / 60)} ثانیه</li>`;
                    } else {
                        infoHTML += `<li style="color:#ff5252;">اضافه بار EX: آماده (همگام‌سازی بارگذاری)</li>`;
                    }
                }
            }
            if (tower.type === 'heavyWeapons') {
                if (currentStats.missileDamage) {
                    ...
                    infoHTML += `<li><span style="color:#ff8a80;font-weight:bold;">آسیب موشک:</span> ${mDmgStr}</li>`;
                    infoHTML += `<li><span style="color:#ff8a80;">شعاع انفجار موشک:</span> ${tower.missileBlastRadius}</li>`;
                    infoHTML += `<li><span style="color:#ff8a80;">فاصله موشک:</span> ${(tower.missileFireRate / 60).toFixed(2)}s</li>`;
                    infoHTML += `<li><span style="color:#ff8a80;">برد موشک:</span> ${missRange} (قفل روی بیشترین جان)</li>`;
                }
            }
            if (tower.type === 'boomerang') {
                infoHTML += `<li>مدت ماندگاری: ${(currentStats.lingerTime / 60).toFixed(1)}s</li>`;
                infoHTML += `<li>برخورد در حین پرواز 3 بار / در حالت ماندگاری هر 0.175 ثانیه</li>`;
                infoHTML += `<li>هر تیغه حداکثر 5 ضربه بحرانی</li>`;
                if (tower.level === 4) {
                    infoHTML += `<li><span style="color:gold;font-weight:bold;">EX:</span> پرواز سریع‌تر؛ آسیب 2 برابر به دشمنان کند/یخ‌زده/سوزان/گیج</li>`;
                }
            }
            if (tower.type === 'frostPunish') {
                infoHTML += `<li><span style="color:#4fc3f7;">مجازات:</span> برخورد با دشمن دارای کندی بالا باعث آسیب منطقه‌ای می‌شود</li>`;
            }
            if (tower.type === 'electricCore') {
                const baseBonus = Math.round((tower.buff - 1) * 100);
                const activeBonus = Math.round(baseBonus * 3);
                infoHTML += `<li>افزایش سرعت حمله: +${tower.skillActiveTimer > 0 ? activeBonus : baseBonus}%</li>`;
                if (tower.level >= 1) infoHTML += `<li>اورکلاک: ${tower.skillActiveTimer > 0 ? `فعال ${Math.ceil(tower.skillActiveTimer / 60)} ثانیه` : tower.skillCooldown > 0 ? `خنک‌سازی ${Math.ceil(tower.skillCooldown / 60)} ثانیه` : 'آماده'}</li>`;
            }
            if (tower.type === 'musicStand' && tower.level >= 2) {
                infoHTML += `<li>الکترو: ${tower.skillActiveTimer > 0 ? `دو برابر ${Math.ceil(tower.skillActiveTimer / 60)} ثانیه` : tower.skillCooldown > 0 ? `خنک‌سازی ${Math.ceil(tower.skillCooldown / 60)} ثانیه` : 'آماده'}</li>`;
            }
            if (tower.type === 'militaryBase') {
                infoHTML += `<li>نیروی بعدی: ${Math.ceil(tower.spawnCooldown / 60)} ثانیه</li>`;
                if (tower.level === 4 && tower.tankSpawnCooldown !== undefined) {
                    infoHTML += `<li>تانک بعدی: ${Math.ceil(tower.tankSpawnCooldown / 60)} ثانیه</li>`;
                }
            }
            ...
            if (level < data.levels.length - 1) {
                ...
                let upgradeText = 'ارتقا';
                let limitReached = false;
                let specialRequirementMet = true;

                if (tower.type === 'thiefClaw' && level === 3) {
                    const stolenAmount = tower.goldStolen || 0;
                    infoHTML += `<li style="color: gold; font-weight: bold;">مجموع دزدیده شده: ${stolenAmount}/1500</li>`;
                ...
                if (tower.type === 'pursuit' && level === 3) {
                    ...
                    infoHTML += `<li style="color: gold; font-weight: bold;">تعداد تعقیب‌گر سطح 4: ${lv4Count}/3</li>`;
                    if (lv4Count < 3) {
                        specialRequirementMet = false;
                    }
                }
                ...
                if (isExTower && level === 3) {
                    ...
                    upgradeText = `ارتقا EX (${exCount}/${data.exLimit || '∞'})`;
            ...
            if (hasMusicSkill) {
                const label = tower.skillActiveTimer > 0 ? `الکترو فعال (${Math.ceil(tower.skillActiveTimer / 60)}s)` : tower.skillCooldown > 0 ? `خنک‌سازی الکترو (${Math.ceil(tower.skillCooldown / 60)}s)` : 'الکترو';
                infoHTML += `<button class="action-btn" data-tower-action="music-skill" ${tower.skillCooldown > 0 ? 'disabled' : ''}>${label}</button>`;
            }
            if (hasElectricSkill) {
                const label = tower.skillActiveTimer > 0 ? `اورکلاک فعال (${Math.ceil(tower.skillActiveTimer / 60)}s)` : tower.skillCooldown > 0 ? `خنک‌سازی اورکلاک (${Math.ceil(tower.skillCooldown / 60)}s)` : 'اورکلاک';
            ...
            if (hasPursuitOverload) {
                const label = tower.overloadActive ? `اضافه بار در حال شلیک (${tower.overloadMissilesRemaining} عدد)` : tower.skillCooldown > 0 ? `خنک‌سازی اضافه بار (${Math.ceil(tower.skillCooldown / 60)}s)` : 'اضافه بار';
            ...
            if (upgradeButtonHTML) {
                ...
                infoHTML += `<button class="action-btn" id="sell-btn" data-tower-action="sell">فروش (<span class="cost">${tower.sell()}</span>)</button></div>`;
            }
...
function translateDestroyerState(state) {
    ...
    return states[state] || 'ناشناخته';
}

function translateStat(stat) {
    const translations = { damage: 'آسیب', range: 'برد', fireRate: 'نرخ شلیک', cost: 'قیمت', slow: 'کندی', blastRadius: 'شعاع انفجار', minDamage: 'حداقل آسیب', maxDamage: 'حداکثر آسیب', rampUpTime: 'زمان شارژ', buff: 'افزایش سرعت حمله', targets: 'تعداد اهداف', stun: 'گیج‌کنندگی', attacksForGold: 'تعداد حمله/پول', goldPerProc: 'پول در هر بار', upgradeDiscount: 'تخفیف ارتقا', rangeBuff: 'افزایش برد', spawnRate: 'سرعت تولید', spawnCount: 'تعداد تولید', attacksBeforeOverheat: 'حمله قبل از داغی', shotsPerRound: 'شلیک در هر دور', goldPerWave: 'درآمد هر موج', pushback: 'فاصله عقب‌راندن', moneyMultiplier: 'ضریب پول اضافی', baseGold: 'پول پایه', tankSpawnRate: 'سرعت تولید تانک', tankSpawnCount: 'تعداد تانک',chainTargets: 'اهداف زنجیره‌ای', chainRadius: 'برد زنجیره‌ای' ,damageBuff: 'افزایش آسیب', salvoCount: 'تعداد تیراندازی پیاپی',salvoInterval: 'فاصله تیراندازی پیاپی',Infinity:'بی‌نهایت', beamSpread: 'پراکندگی پرتو', burnPercent: 'آسیب سوختن', bossBurnPercent: 'آسیب سوختن به رئیس', critChance: 'احتمال ضربه بحرانی', reloadTime: 'زمان بارگذاری', missileCount: 'تعداد موشک' };
...
function gameOver(isVictory) {
    ...
    if (isVictory) {
        ...
        modalTitle.textContent = isReplayMode ? 'بازپخش نماینده کامل شد' : 'پیروزی!';
        modalText.textContent = isReplayMode ? `همه ${wave} موج با موفقیت بازپخش شد.` : `تبریک! شما تمام ${wave} موج را با موفقیت دفاع کردید!`;
        if (lastVictoryResult) {
            const bonusPercent = Math.round(lastVictoryResult.expertBonus * 100);
            scoreSummary.innerHTML = `<span class="score-total">امتیاز کل این بازی: ${lastVictoryResult.totalScore.toLocaleString()}</span><div class="score-breakdown"><span>امتیاز سرعت <b>${lastVictoryResult.speedScore.toLocaleString()}</b></span><span>امتیاز پایگاه <b>${lastVictoryResult.baseScore.toLocaleString()}</b></span><span>ضریب متخصص <b>+${bonusPercent}%</b></span></div>`;
            scoreSummary.style.display = 'block';
        }
        uploadScoreBtn.style.display = isReplayMode || isTestMode ? 'none' : 'inline-block';
        uploadScoreBtn.disabled = false;
        uploadScoreBtn.textContent = 'ارسال به رتبه‌بندی';
    } else {
        lastVictoryResult = null;
        modalTitle.textContent = "پایان بازی";
        modalText.textContent = `پایگاه شما در موج ${wave} نابود شد.`;
...
function setupTestField() {
    ...
    hpDisplay.textContent = 'DPS کل فعلی: 0.00';
    waveDisplay.textContent = 'DPS این برج: در انتظار انتخاب';
    startWaveBtn.style.display = 'block';
    startWaveBtn.textContent = 'خروج از زمین تست';
