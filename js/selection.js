...
function initializeSelectionScreen() {
    ...
    for (const type in TOWER_DATA) {
        ...
        card.addEventListener('mouseenter', () => {
            ...
            if (data.limit) {
                infoHTML += `<p style="margin-top: 10px; color: var(--primary-color); font-weight: bold;">محدودیت ساخت: ${data.limit}</p>`;
            }
            if (data.exDescription) {
                infoHTML += `<hr style="border-color: var(--border-color); margin: 10px 0;">`;
                infoHTML += `<p style="color: gold; font-weight: bold;">ویژگی EX: ${data.exDescription}</p>`;
                if (data.exLimit && data.exLimit < 99) {
                    infoHTML += `<p style="color: #4dd0e1;">محدودیت ارتقا EX: ${data.exLimit}</p>`;
                }
            }
...
function updateStartButtonState() {
    ...
    if (count >= 1 && count <= 7) {
        ...
        selectMapBtn.textContent = `انتخاب نقشه (${count}/7)`;
        startTestBtn.disabled = false;
    } else {
        selectMapBtn.disabled = true;
        startTestBtn.disabled = true;
        if (count === 0) {
            selectMapBtn.textContent = 'لطفاً حداقل ۱ برج انتخاب کنید';
        } else {
            selectMapBtn.textContent = 'حداکثر ۷ برج می‌توانید انتخاب کنید';
        }
    }
}
